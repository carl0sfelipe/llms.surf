...
| 53 | veredito de run só existe na superfície canônica do próprio workdir (status --task), e não existem dois dispatches vivos no mesmo workdir; log existente não é log DO seu run | **R** | permanece — MECANISMOS pendentes declarados na própria regra (flock single-flight por workdir, status --task canônico, imprint sha256 no green); interage_com 24, 31, 42, 44 e 26. Incidente: `incidents/2026-08-27-dogfooding-tres-despachos-no-mesmo-workdir-e-veredito-lido-grepa-crud.md` |
