...
ORBE="${ORBE_ROOT:-$HOME/orbe.live-imports}"
...
cf-probe.sh [opções]
  ...
  --orbe PATH        raiz do orbe.live-imports (env ORBE_ROOT)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --orbe)     ORBE="$2"; CF="$ORBE/apps/content-factory"; shift 2 ;;
    --out)      OUT="$2"; shift 2 ;;
...
echo "cf-probe: oracfit=$ORACFIT orbe=$ORBE" >&2
...
# =================================================== 5. oracle_engine =======
SPEC_BEELINK="$ORACFIT/specs/niche-beelink-ser-6800u-vale-a-pena.md"
