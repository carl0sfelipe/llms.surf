...
titulo: Oracfit soldado no Orbe — corte sem categoria specs
data: 2026-08-13
recorrivel: sim
regra: 52
status: promovido
interage_com: |
  2026-07-28-versao-publicada-sem-commit-fonte-de-0-1 (desacoplar Orbe↔medusa-br;
...
# Oracfit soldado no Orbe — corte sem categoria specs
...
3. O modo de produto `content_factory` assume a árvore do Orbe
...
|---|---|---|
| 2026-08-10 | `d01774b` release v1.9.0 | `core/modes/content_factory.yaml` + os dois testes com path absoluto do Orbe. O veneno T4 era real (instinto certo); o que foi versionado foi o **caminho da máquina**, não uma fixture em `tests/fixtures/`. |
| 2026-08-10 | `d3b8d6f` `content(<host-local>):` | 10 specs de nicho do catálogo Orbe |
...
| 2026-08-11 | incidente de uso | 22 despachos, workdir `<repo-cliente>.live-imports`, extração de 5.758 registros / 3.567 únicos. O JSON de saída ficou no Orbe; as **specs com o texto-fonte** ficaram aqui. |
...
ORBE="${ORBE_ROOT:-<home-do-dono>/<repo-cliente>.live-imports}"
```
...
Não é "esquecemos de desacoplar o Orbe do medusa-br". Aquele
desacoplamento é outra guerra (incidente `2026-07-28-versao-publicada-…`,
`kit-storefront` no consumidor Orbe vs fonte em `medusa-br-framework`).
O resíduo dela já mora *neste* `incidents/` — outro sintoma da mesma
inversão: o diário da oficina Orbe foi para o corte do orquestrador.
...
4. **Fixture dos testes T4** — `tests/fixtures/t4-poison-nintendo-switch-2.yaml`
   ...
   por `$REPO_ROOT`. Os dois passam sem o Orbe no disco (7/0 e 6/0).
...
(56 arquivos, ~988 KB) **movido para o repo do Orbe**, ao lado do resto do
...
"ORBE" em caixa alta sobrando no postmortem da regra 43 (a anonimização
só trocava "Orbe"); (2) `incidents/evidence/` inteira no corte — 1,7MB de
