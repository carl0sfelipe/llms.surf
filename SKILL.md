...
**53. VEREDITO DE RUN SÓ EXISTE NA SUPERFÍCIE CANÔNICA DO PRÓPRIO WORKDIR — E NÃO EXISTEM DOIS DISPATCHES VIVOS NO MESMO WORKDIR. Em 2026-08-27 três despachos (dogfood-03/03c/03d) sobrepuseram-se no mesmo workdir por 7+ minutos — dois agentes escrevendo o MESMO artefato — e o operador leu pass num grep cru de run_finished que era do run ANTERIOR (dogfood-02): 30min de diagnóstico contra estado inexistente e uma noite de traduções perdidas do worktree. Em 2026-08-29 a classe repetiu em outra camada: revisão de implementação executada num checkout onde o trabalho não vivia — veredito de ausência lido fora da superfície certa. Log existente NÃO é log DO seu run; branch revisada NÃO é branch onde o trabalho está. MECANISMOS (todos PENDENTES — regra 32, dívida declarada até entrar): (1) flock single-flight por workdir no entrypoint de dispatch — segundo dispatch no mesmo workdir RECUSA em vez de sobrepor; (2) oracfit status --task como superfície canônica de veredito — grep cru em events.jsonl deixa de ser caminho de decisão; (3) imprint sha256 do tree no fechamento verde — green sem imprint não é green. INTERAGE_COM: reforça 24 (captura mentiu sobre o que rodou), 31 (pass lido sem trabalho atrás), 42 (sinal de vida lido errado — aqui, log vivo não prova que é o SEU log), 44 (veredito exige campo canônico na superfície certa); combina com 26 (ausência só se prova com busca completa — a busca parcial de 2026-08-29 alegou ausência universal de um pack que vivia em outro checkout).** (incidente: `incidents/2026-08-27-dogfooding-tres-despachos-no-mesmo-workdir-e-veredito-lido-grepa-crud.md`)
