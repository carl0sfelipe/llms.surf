...
**53. VEREDITO DE RUN SÓ EXISTE NA SUPERFÍCIE CANÔNICA DO PRÓPRIO WORKDIR — E NÃO EXISTEM DOIS DISPATCHES VIVOS NO MESMO WORKDIR. Em 2026-08-27 três despachos (dogfood-03/03c/03d) sobrepuseram-se no mesmo workdir por 7+ minutos — dois agentes escrevendo o MESMO artefato — e o operador leu pass num grep cru de run_finished que era do run ANTERIOR (dogfood-02): 30min de diagnóstico contra estado inexistente e uma noite de traduções perdidas do worktree. Em 2026-08-29 a classe repetiu em outra camada: revisão de implementação executada num checkout onde o trabalho não vivia — veredito de ausência lido fora da superfície certa. Log existente NÃO é log DO seu run; branch revisada NÃO é branch onde o trabalho está. MECANISMOS: (1) CONSTRUÍDO 2026-08-29 (spec S7): single-flight por workdir — oracfit_run_lock_acquire/release em bin/lib-oracfit-root.sh (mkdir atômico + pid file; PORTÁTIL, sem flock — hosts incluem macOS/bash 3.2), aplicado nos DOIS entrypoints (bin/dispatch-mode.sh e bin/dispatch-stages.sh, antes de qualquer preflight, liberado via trap em todo caminho de saída): segundo dispatch com run vivo recusa (exit 6) nomeando pid do dono e caminho do lock, sem ensinar remoção forçada (espírito da 47); lock órfão de pid morto é assumido, não recusado. (2) CONSTRUÍDO 2026-08-29 (spec S7): oracfit status --task <nome> (bin/oracfit cmd_status) — JSON único do run mais recente da task neste workdir; grep cru em events.jsonl deixa de ser caminho de decisão. (3) PENDENTE (regra 32, dívida): imprint sha256 do tree no fechamento verde — green sem imprint não é green. INTERAGE_COM: reforça 24 (captura mentiu sobre o que rodou), 31 (pass lido sem trabalho atrás), 42 (sinal de vida lido errado — aqui, log vivo não prova que é o SEU log), 44 (veredito exige campo canônico na superfície certa); combina com 26 (ausência só se prova com busca completa — a busca parcial de 2026-08-29 alegou ausência universal de um pack que vivia em outro checkout).** (incidente: `incidents/2026-08-27-dogfooding-tres-despachos-no-mesmo-workdir-e-veredito-lido-grepa-crud.md`)

**54. A ALLOWLIST DE PROVIDER (E5-M6/R4) SÓ GOVERNA O CAMINHO FREE: dispatch com model-id EXPLÍCITO (decisão deliberada do operador) grava allowlist_status=fora-do-escopo, nunca violado. O .efetivo passa a carregar a origem (tier:* vs id explícito) e o ledger-finalize decide por ela — sem isso, todo dispatch pago pelo caminho canônico é caluniado de violação. SEM MECANISMO HOJE (campo origem pendente no run-with-fallback) — dívida declarada no incidente 2026-08-30.** (incidente: `incidents/2026-08-30-allowlist-do-free-path-assertiona-dispat.md`)

**55. FALLBACK DE REGISTRY É VALIDADO NO ENCADEAMENTO, NÃO NA FÉ: ref existe (id ou hint) E endpoint responde — fantasma ou endpoint morto PULA pro próximo da cadeia, cadeia vazia = exit 2 limpo na cara do operador. Fallback estático apodrece em silêncio (audit 2026-08-30: 7 fantasmas, 1 morto, 3 viáveis em 11 refs; o morto transformou rate limit temporário em run assassinado por retry mudo). SEM MECANISMO HOJE (checagem pré-fallback pendente no run-with-fallback.sh) — dívida declarada.** (incidente: `incidents/2026-08-30-fallback-do-registry-aponta-endpoint-sus.md`)
