...
|---|---|---|---|
| **D1** | A promessa de 2 minutos é o gate zero: nenhum tráfego sobe antes de S1 verde. | O quickstart do site (`site/app.js`) termina em `bin/llms-surf start`; em HEAD esse comando despachava `tests/fixtures/oracfit-smoke-normal.md` — e `specs/` inteiro ficou fora do corte público (regra 52). Seis pontos do tree apontavam para o arquivo morto: `bin/oracfit` (first-proof), `bin/test-oracfit-tldr.sh:20`, `bin/release-gauntlet-verify.sh:72` **e `:80`** (uma segunda spec morta, `oracfit-smoke-unlock-plan.md`, achada nesta sessão), `tests/test-gui-remote.sh:63`, `tests/test-protected-paths.sh:14`. | As duas smokes recriadas; `check-spec` + `check-oracle` + stub dispatch verdes; `dispatch-stages.sh` agora cria `.dispatch` antes do preflight (mesmo invariante do dispatch-mode). Oracle: S1. |
